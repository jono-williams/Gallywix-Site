...
<body>
    <div class="non-static">
        ...
        <section class="about" id="about">
            ...
            <h2 style="color:orange; text-align: center; opacity: 0.5;font-size: 50px;margin-top:25px;">Found List:</h2>
            <code id="foundSheetNames">&nbsp;</code>
            <h2 style="color:orange; text-align: center; opacity: 0.5;font-size: 50px;margin-top:25px;">Not Found List:</h2>
            <code id="unfoundSheetNames">&nbsp;</code>

                <?php foreach($gtrack as $key => $run) { ?>
                  <table class="gtrack">
                    <thead>
                      <tr>
                        <?php foreach ($run as $key => $value) { if($key) { ?>
                          <th><?=$key?></th>
                        <?php }} ?>
                      </tr>
                    </thead>
      ...
      function fetchmainsapi() {
        ...
        $.ajax({
          ...
          success: function(data) {
            $('#foundSheetNames').html(data.found.join(","));
            $('#unfoundSheetNames').html(data.unfound.join(","));
          }
        })
      }
    </script>
