...
<head>
    ...
    <title>Gallywix | Booster Tracker</title>
    ...
    <div class="non-static">
        ...
        <section class="about" id="about">
            <div class="container" style="margin-bottom: 50px; margin-top: 50px; border-top:2px solid orange; border-bottom:2px solid orange; padding: 15px 0px;">
              <h1 style="color:orange; text-align: center; opacity: 0.5;font-size: 50px;">Booster Tracker</h1>
            </div>
            <div class="container">
              <table class="table responsive" id="dtBasicExample">
                ...
                <tbody>
                  <?php foreach($boosters as $v) { if(@$v['name']) { ?>
                    <tr>
                      <td><?=$v['name']?></td>
                      <td><?=$v['role']?></td>
                      <td><?=number_format($v['count'],0)?></td>
                      <td><?=number_format($v['total'],0)?></td>
                      <td><?=@number_format($v['total']/$v['count'],0)?></td>
                    </tr>
                <?php }} ?>
                </tbody>
              </table>
